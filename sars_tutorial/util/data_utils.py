...
def sequences_to_spotlight_format(sequences, max_seq_len=80):	
    """	
    Convert a list of sequences to Pytorch Spotlight format's	
    :param dataset:	
    :return:	
    """	
    padded = sequences["sequence"].apply(lambda x: pad(x, max_seq_len))	
    return SequenceInteractions(np.array(list(padded.to_numpy())))
...
def print_metrics(metrics, eval_results, rec_name, given_k, look_ahead, step, topn):	
    print('Sequential evaluation for {} (GIVEN_K={}, LOOK_AHEAD={}, STEP={})'.format(rec_name, given_k, look_ahead, step, topn))	
