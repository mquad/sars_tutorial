...
class FPM_recommender_tests(unittest.TestCase):
    ...
    def test_evaluation(self):
        ...
        assert np.array_equal(evaluation.evaluate_sequence(rec1, test_seq, 3, 1, evaluation_functions),
                              np.zeros(len(evaluation_functions)))
        assert np.array_equal(evaluation.evaluate_sequence(rec2, test_seq, 3, 1, evaluation_functions),
                              np.array([0.5, 1]))

        # look ahead = 4 = set evaluation, ground truth less than 4
        assert np.array_equal(evaluation.evaluate_sequence(rec2, test_seq, 3, 4, evaluation_functions),
                              np.array([0.5, 1 / 3]))
        assert np.array_equal(evaluation.evaluate_sequence(rec2, test_seq, 3, 'total', evaluation_functions),
                              np.array([0.5, 1 / 3]))

        assert np.array_equal(evaluation.evaluate_sequence(rec2, [3, 4, 2, 4, 1, 1], 3, 'total', evaluation_functions),
                              np.array([0.5, 0.5]))
        ...
        assert np.array_equal(evaluation.set_evaluation(rec3, db, 3, 'total', evaluation_functions),
                              np.array([2 / 3, 1 / 2]))

        ##single sequential evaluation
        assert np.array_equal(evaluation.sequence_sequential_evaluation(rec2, [7, 8, 9, 3, 4, 2, 1, 5, 1, 6, 7, 8], 3,
                                                                        evaluation_functions), np.array([1 / 6, 1 / 3]))
        assert np.array_equal(evaluation.sequence_sequential_evaluation(rec2, [7, 8, 9, 3, 4, 2, 1, 5, 1, 6, 4, 8], 3,
                                                                        evaluation_functions), np.array([1 / 3, 2 / 3]))

        # sequential evaluation of db
        db = [[7, 8, 9, 3, 4, 2, 1, 5, 1, 6, 7, 8],
              [7, 8, 9, 3, 4, 2, 1, 5, 1, 6, 4, 8]]

        assert np.array_equal(evaluation.sequential_evaluation(rec2, db, 3, 'total', evaluation_functions),
                              np.array([3 / 12, 0.5]))
    ...
    def test_n_length_paths(self):
        ...
        assert sorted(t.get_paths_tag(p2)) == sorted(
            [['root', 1, 2], ['root', 1, 3], ['root', 2, 3], ['root', 2, 1], ['root', 3, 4], ['root', 3, 5],
             ['root', 4, 2]])
        assert sorted(t.get_paths_tag(p3)) == sorted(
            [['root', 1, 3, 1], ['root', 1, 3, 6], ['root', 1, 3, 2], ['root', 3, 4, 2], ['root', 3, 4, 1],
             ['root', 4, 2, 1]])
